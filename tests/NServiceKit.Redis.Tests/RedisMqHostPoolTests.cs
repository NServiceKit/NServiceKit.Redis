...
namespace NServiceKit.Redis.Tests
{
    ...
    public class RedisMqHostPoolTests
	{
        ...
        private static void Publish_4_Rot13_messages(IMessageQueueClient mqClient)
        {
            ...
            mqClient.Publish(new Rot13 { Value = "NServiceKit" });
		...
		public void Does_retry_messages_with_errors_by_RetryCount()
		{
			...
			var mqHost = CreateMqHostPool(retryCount);
		...
		public void Can_receive_and_process_standard_request_reply_combo()
		{
			...
			var dto = new Hello { Name = "NServiceKit" };
			mqClient.Publish(dto);

			Thread.Sleep(1000);

			Assert.That(messageReceived, Is.EqualTo("Hello, NServiceKit"));
		}
	}
}